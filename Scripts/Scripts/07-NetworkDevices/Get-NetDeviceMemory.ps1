...
.DESCRIPTION
    Captures memory utilisation output, extracting the used-percentage where
    the platform reports it in a stable format.
...
.EXAMPLE
    .\Get-NetDeviceMemory.ps1 -DeviceName SW01 -Vendor cisco-ios

    Memory capture.

.EXAMPLE
    .\Get-NetDeviceMemory.ps1 -DeviceName SW01,SW02 -Vendor cisco-nxos -RawOutputPath .\\captures

    Two devices, raw kept.

.NOTES
    Source use case      : #3 - Device Memory Details
    ...
    Free memory on a network device is not the same measurement as on a
    server - a device that has allocated most of its processor pool is
    usually healthy, not about to fail. Treat the percentage as a trend
    line, not a threshold. This script captures RAW device output and does
...
begin {
    ...
    $scriptName = 'Get-NetDeviceMemory'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #3 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Device Memory Details'
