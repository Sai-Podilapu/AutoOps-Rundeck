...
.DESCRIPTION
    Reports installed hardware, module inventory and environmental readings -
    power supplies, fans and temperature - so a failed redundant component is
    visible before the second one goes.
...
.EXAMPLE
    .\Get-NetDeviceHardware.ps1 -DeviceName SW01 -Vendor cisco-ios -RawOutputPath .\\captures
...
.NOTES
    Source use case      : #5 - Device Hardware Details
    Category             : Network Devices
    Technology           : SNMP / REST
    Difficulty           : Low
    Agent possible       : Yes
    ...
    A failed redundant power supply produces no outage and no alert on
    many platforms; it just quietly removes the redundancy. That is the
    finding this capture is for. This script captures RAW device output
    and does not parse it. That is deliberate and is the master prompt's
    own guidance: turning multi-vendor CLI text into structured data is
    what Netmiko and NAPALM exist for, and a PowerShell reimplementation
    of their template library would be a large, fragile and worse copy.
    The verbatim capture is the product; feed it to a parser that already
    knows these formats.
...
begin {
    ...
    $scriptName = 'Get-NetDeviceHardware'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #5 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Device Hardware Details'
