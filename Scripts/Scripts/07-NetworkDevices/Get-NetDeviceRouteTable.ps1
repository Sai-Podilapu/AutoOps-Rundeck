...
.DESCRIPTION
    Reports the device's routing table in full, giving the record needed to
    explain why traffic went where it did.
...
.EXAMPLE
    .\Get-NetDeviceRouteTable.ps1 -DeviceName CORE01 -Vendor cisco-ios -RawOutputPath .\\captures

    Full route table captured to disk.

.EXAMPLE
    .\Get-NetDeviceRouteTable.ps1 -DeviceName CORE01 -Vendor juniper-junos

    Junos route table.

.NOTES
    Source use case      : #12 - Display IP Route (general)
    ...
    A full routing table from a core device can run to thousands of lines.
    Use -RawOutputPath to keep it on disk rather than in console output,
    and use case #13 when you want one prefix. This script captures RAW
...
begin {
    ...
    $scriptName = 'Get-NetDeviceRouteTable'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #12 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Display IP Route (general)'
