...
.DESCRIPTION
    Reports the process list with its resource consumption, which is the next
    question after a device shows high CPU: what is using it.
...
.EXAMPLE
    .\Get-NetDeviceProcess.ps1 -DeviceName SW01 -Vendor cisco-ios

    Process list sorted by CPU and by memory.

.EXAMPLE
    .\Get-NetDeviceProcess.ps1 -DeviceName SW01 -Vendor juniper-junos -RawOutputPath .\\captures

    Junos process detail.

.NOTES
    Source use case      : #16 - View Process ID & Resource Consumption
    ...
    On many platforms a high-CPU process is a symptom of something else -
    an interrupt storm, a routing reconvergence - rather than the cause.
    The process list narrows the search; it rarely ends it. This script
    captures RAW device output and does not parse it. That is deliberate
    and is the master prompt's own guidance: turning multi-vendor CLI text
    into structured data is what Netmiko and NAPALM exist for, and a
    PowerShell reimplementation of their template library would be a
    large, fragile and worse copy. The verbatim capture is the product;
    feed it to a parser that already knows these formats.
...
begin {
    ...
    $scriptName = 'Get-NetDeviceProcess'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #16 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'View Process ID & Resource Consumption'
