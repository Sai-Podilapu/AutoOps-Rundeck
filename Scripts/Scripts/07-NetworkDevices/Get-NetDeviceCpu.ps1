...
.DESCRIPTION
    Captures CPU utilisation output. For the platforms whose format is
    unambiguous the headline percentage is extracted as well; for the rest the
...
.EXAMPLE
    .\Get-NetDeviceCpu.ps1 -DeviceName SW01 -Vendor cisco-ios

    CPU capture with the headline percentage extracted.

.EXAMPLE
    .\Get-NetDeviceCpu.ps1 -DeviceName SW01,SW02 -Vendor cisco-nxos -OutputFormat HTML

    Two NX-OS devices as HTML.

.NOTES
    Source use case      : #2 - Device CPU Details
    Category             : Network Devices
    Technology           : SNMP / Netmiko
    Difficulty           : Low
    Agent possible       : Yes
    ...
    The five-second CPU figure spikes for reasons that do not matter. The
    five-minute average is the one worth alerting on, which is why the raw
    output - containing all three - is kept rather than reduced to a
    single number. This script captures RAW device output and does not
    parse it. That is deliberate and is the master prompt's own guidance:
    turning multi-vendor CLI text into structured data is what Netmiko and
    NAPALM exist for, and a PowerShell reimplementation of their template
    library would be a large, fragile and worse copy. The verbatim capture
    is the product; feed it to a parser that already knows these formats.
...
begin {
    ...
    $scriptName = 'Get-NetDeviceCpu'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #2 (Network Devices)'
...
process {
    try {
        ...
        foreach ($device in $DeviceName) {
            ...
            # Only parsed where the format is stable and documented. Everything else
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Device CPU Details'
