...
.DESCRIPTION
    Searches the device log buffer for a keyword and returns the most recent
    matches - the usual first step when an interface, a neighbour or an error
    code needs its history.
...
.PARAMETER Keyword
    Keyword passed to the device log filter.

.PARAMETER LineCount
    How many trailing matches to return.
...
.EXAMPLE
    .\Get-NetDeviceLogSearch.ps1 -DeviceName SW01 -Vendor cisco-ios -Keyword 'GigabitEthernet1/0/24'

    Every recent log line mentioning one port.

.EXAMPLE
    .\Get-NetDeviceLogSearch.ps1 -DeviceName SW01 -Keyword ERROR -LineCount 50

    Last 50 lines containing ERROR.

.NOTES
    Source use case      : #18 - Display Last 100 Logs w/ Keyword Filter
    Category             : Network Devices
    Technology           : Netmiko / SSH / grep
    Difficulty           : Low
    Agent possible       : Yes
    ...
    The keyword is passed to the device's own filter, so the device does
    the work rather than shipping its whole buffer over SSH. The match is
    the platform's, not a PowerShell regex: case sensitivity and pattern
    syntax are the device's. This script captures RAW device output and
    does not parse it. That is deliberate and is the master prompt's own
...
begin {
    ...
    $scriptName = 'Get-NetDeviceLogSearch'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #18 (Network Devices)'
...
process {
    try {
        ...
        # The device's own filter runs the match, so its buffer is not shipped whole
        ...
        foreach ($device in $DeviceName) {
            ...
            # Trimmed here, not on the device: platforms disagree on whether a
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Display Last 100 Logs w/ Keyword Filter'
