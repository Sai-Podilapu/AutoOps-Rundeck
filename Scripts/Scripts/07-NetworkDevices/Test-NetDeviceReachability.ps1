...
.DESCRIPTION
    Executes ping and traceroute from the device itself rather than from the
    automation host. That distinction is the whole point: reachability from
    the device's own routing table and source interface is what a network
    fault report needs.
...
.PARAMETER Target
    Address or hostname to test reachability to.
...
.EXAMPLE
    .\Test-NetDeviceReachability.ps1 -DeviceName SW01 -Vendor cisco-ios -Target 8.8.8.8

    Ping and traceroute to a public address from the switch.

.EXAMPLE
    .\Test-NetDeviceReachability.ps1 -DeviceName SW01 -Target 10.1.1.1 -SkipTraceroute

    Ping only.

.NOTES
    Source use case      : #11 - Traceroute & Ping Test
    Category             : Network Devices
    Technology           : Python / Netmiko
    Difficulty           : Low
    Agent possible       : Yes
    ...
    Guardrails (col L)   : "Diagnostic only"
    ...
    Running the test from the device, not from here, is deliberate - a
    path that works from the automation host says nothing about the path
    the device would take. The ping success rate is extracted where the
    platform reports it in a stable format. This script captures RAW
    device output and does not parse it. That is deliberate and is the
    master prompt's own guidance: turning multi-vendor CLI text into
    structured data is what Netmiko and NAPALM exist for, and a PowerShell
    reimplementation of their template library would be a large, fragile
    and worse copy. The verbatim capture is the product; feed it to a
    parser that already knows these formats.
...
begin {
    ...
    $scriptName = 'Test-NetDeviceReachability'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #11 (Network Devices)'
...
process {
    try {
        ...
        # Built from -Target at runtime rather than being a static table.
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Traceroute & Ping Test'
