...
.DESCRIPTION
    Runs the CPU, memory and filesystem commands in one pass so a monitoring
    record covers all three from the same moment, rather than three captures
    taken minutes apart.
...
.EXAMPLE
    .\Get-NetDeviceResourceUtilization.ps1 -DeviceName SW01 -Vendor cisco-ios -OutputFormat HTML

    All three readings for one device.

.EXAMPLE
    .\Get-NetDeviceResourceUtilization.ps1 -DeviceName SW01,SW02,SW03 -RawOutputPath .\\captures

    Fleet snapshot, raw kept.

.NOTES
    Source use case      : #14 - Resource Utilization - CPU/Mem/Disk
    Category             : Network Devices
    Technology           : SNMP / Prometheus
    Difficulty           : Low
    Agent possible       : Yes
    ...
    Guardrails (col L)   : "Monitoring only"
    ...
    A device out of flash storage cannot save configuration or write a
    crash dump, and nothing about its CPU or memory reading will hint at
    it. That is why storage is captured alongside, not separately. This
...
begin {
    ...
    $scriptName = 'Get-NetDeviceResourceUtilization'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #14 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Resource Utilization - CPU/Mem/Disk'
