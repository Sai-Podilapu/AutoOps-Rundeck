...
.DESCRIPTION
    Runs the vendor's version and inventory commands on each device and
    captures the output verbatim, giving a point-in-time record of model,
    serial, software version and installed modules.
...
.EXAMPLE
    .\Get-NetDeviceInventory.ps1 -DeviceName FW01 -Vendor generic -Command 'get system status'

    A platform with no built-in command set, driven explicitly.

.NOTES
    Source use case      : #1 - Device Inventory Details
    ...
    Guardrails (col L)   : "Read-only show commands"
    ...
    This script captures RAW device output and does not parse it. That is
...
begin {
    ...
    $scriptName = 'Get-NetDeviceInventory'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #1 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Device Inventory Details'
