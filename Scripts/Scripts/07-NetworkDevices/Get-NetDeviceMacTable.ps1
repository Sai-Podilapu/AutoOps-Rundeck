...
.DESCRIPTION
    Reports the MAC address table, which answers "what is plugged into this
    port" and, read the other way, "which port is this device on".
...
.EXAMPLE
    .\Get-NetDeviceMacTable.ps1 -DeviceName SW01 -Vendor cisco-ios

    Full MAC table.

.EXAMPLE
    .\Get-NetDeviceMacTable.ps1 -DeviceName SW01 -Vendor cisco-ios -Command 'show mac address-table address 0011.2233.4455'

    One specific MAC.

.NOTES
    Source use case      : #7 - Display MAC-to-Interface Mapping
    ...
    The MAC table only shows what the switch has heard recently - a device
    that has been quiet past the ageing time is absent from it despite
    being connected. An empty result for a port is not evidence that
    nothing is attached. This script captures RAW device output and does
...
begin {
    ...
    $scriptName = 'Get-NetDeviceMacTable'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #7 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Display MAC-to-Interface Mapping'
