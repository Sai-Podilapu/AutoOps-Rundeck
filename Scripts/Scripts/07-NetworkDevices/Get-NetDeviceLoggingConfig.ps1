...
.DESCRIPTION
    Reports where a device is sending its logs and at what severity. A device
    logging only to its local buffer is a gap: the buffer is lost on reload,
    which is exactly when the logs matter.
...
.EXAMPLE
    .\Get-NetDeviceLoggingConfig.ps1 -DeviceName SW01 -Vendor cisco-ios

    Logging configuration for one device.

.EXAMPLE
    .\Get-NetDeviceLoggingConfig.ps1 -DeviceName SW01,SW02,SW03 -OutputFormat HTML

    Logging destinations across a fleet.

.NOTES
    Source use case      : #4 - Device Logging Details
    Category             : Network Devices
    Technology           : Syslog / SNMP
    Difficulty           : Low
    Agent possible       : Yes
    ...
    The commands here deliberately filter to the configuration header
    rather than dumping the log buffer - use case #17 does that. This
...
begin {
    ...
    $scriptName = 'Get-NetDeviceLoggingConfig'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #4 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Device Logging Details'
