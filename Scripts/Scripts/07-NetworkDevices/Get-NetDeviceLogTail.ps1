...
.DESCRIPTION
    Retrieves the device log buffer and returns the last N lines. The tail is
    applied here rather than on the device, so the count means the same thing
    on every platform.
...
.EXAMPLE
    .\Get-NetDeviceLogTail.ps1 -DeviceName SW01 -Vendor cisco-ios

    Last 100 log lines.

.EXAMPLE
    .\Get-NetDeviceLogTail.ps1 -DeviceName SW01 -LineCount 500 -RawOutputPath .\\captures

    A deeper tail, kept on disk.

.NOTES
    Source use case      : #17 - Display Last 100 Log Lines
    Category             : Network Devices
    Technology           : Netmiko / SSH
    Difficulty           : Low
    Agent possible       : Yes
    ...
    Platforms differ in whether they support a "last N" argument at all,
    and in whether it counts from the newest or oldest entry. Taking the
    tail locally makes -LineCount mean one thing everywhere. The device
    buffer is finite and is lost on reload - if these lines matter, the
    device should be sending them to a syslog server, which use case #4
    checks. This script captures RAW device output and does not parse it.
    That is deliberate and is the master prompt's own guidance: turning
...
begin {
    ...
    $scriptName = 'Get-NetDeviceLogTail'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #17 (Network Devices)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Display Last 100 Log Lines'
