<#
.SYNOPSIS
    Reports Send on Behalf Of delegations across mailboxes.

.DESCRIPTION
    Lists every mailbox with a Send on Behalf grant and who holds it, for
    delegation review.
...
.EXAMPLE
    .\Get-ExoSendOnBehalf.ps1 -Mailbox shared@contoso.com

    Delegations on one mailbox.

.EXAMPLE
    .\Get-ExoSendOnBehalf.ps1 -OutputFormat CSV

    Tenant-wide delegation export.

.NOTES
    Source use case      : #8 - Check 'On Behalf Of' Permissions
...
begin {
    ...
    $scriptName = 'Get-ExoSendOnBehalf'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #8 (Exchange & O365)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Check ''On Behalf Of'' Permissions'
