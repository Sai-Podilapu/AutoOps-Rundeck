<#
.SYNOPSIS
    Reports who has permissions on a mailbox.

.DESCRIPTION
    Lists every non-inherited mailbox permission with the trustee and access
    rights, so an access review can answer "who can read this mailbox" without
    opening the admin centre.
...
.PARAMETER IncludeInherited
    Include inherited permissions, which are usually noise.
...
.EXAMPLE
    .\Get-ExoMailboxPermission.ps1 -Mailbox shared@contoso.com

    Permissions on one mailbox.

.EXAMPLE
    .\Get-ExoMailboxPermission.ps1 -OutputFormat CSV

    Tenant-wide permission export for an access review.

.NOTES
    Source use case      : #5 - Check Permissions on Mailbox
...
begin {
    ...
    $scriptName = 'Get-ExoMailboxPermission'
    Write-AutomationLog -ScriptName $scriptName -Level INFO -Message 'START - use case #5 (Exchange & O365)'
...
process {
    ...
    $null = $candidates | Export-AutomationResult -OutputFormat $OutputFormat -Path $OutputPath -Title 'Check Permissions on Mailbox'
