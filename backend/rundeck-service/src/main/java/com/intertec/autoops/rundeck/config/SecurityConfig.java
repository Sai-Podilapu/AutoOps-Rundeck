...
import com.intertec.autoops.rundeck.security.InternalTokenFilter;
import com.intertec.autoops.rundeck.security.RestAuthEntryPoint;
...
/**
 * Stateless resource server, identical in rules to workflow-service's and
 * core-service's: the same auth-service RS256 tokens and the same
 * VIEWER-is-read-only edge check.
 *
 * <p>One rule is stricter here. Running a Rundeck job executes commands on a
 * customer's fleet, so {@code POST /api/rundeck/**} is a WRITE even though the
 * job it triggers may only read — a VIEWER may browse jobs and watch logs, and
 * may not press run. That is why dispatch lives under POST and nothing here
 * accepts a run request on a GET, however convenient Rundeck's own API makes it.
 *
 * <p>{@code /internal/**} carries no user token at all. Those endpoints exist
 * for core-service (a Rundeck job used as a workflow step), are never routed by
 * the gateway, and are guarded by {@link InternalTokenFilter}.
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
    ...
    @Bean
    public JwtDecoder jwtDecoder(RundeckProperties properties) {
