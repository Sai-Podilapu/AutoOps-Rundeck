 ...
 * core-service and workflow-service use. The tenant is never sent:
 * subscription-service reads it from the END USER's bearer token, so this
 * service can only ever ask about the caller's own tenant.
 *
 * <p><strong>FAIL-CLOSED</strong> by default when subscription-service is
 * unreachable. Reads never pass through here — a tenant can always see its own
 * connections and watch a running execution, even mid-outage.
 */
@Component
