...
import com.intertec.autoops.plugin.security.InternalTokenFilter;
import com.intertec.autoops.plugin.security.RestAuthEntryPoint;
...
/**
 * Stateless resource server, identical in rules to workflow-service's: the
 * same auth-service RS256 tokens, the same VIEWER-is-read-only edge check,
 * and no public business endpoints.
 *
 * <p>{@code /internal/**} is the one exception — it carries no user token at
 * all. Those endpoints exist for core-service's lifecycle events, are never
...
public class SecurityConfig {
    ...
    @Bean
    public JwtDecoder jwtDecoder(PluginProperties properties) {
